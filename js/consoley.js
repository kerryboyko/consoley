...
var jsonLoad = function() {
    listOfPackages = $.getJSON(
        "http://brianboyko.github.io/consoley/packages.json", function(
            result) {});
};
...
var allPackages = function(list) { // takes an array of objects from JSON
    ...
    list.forEach(function() {
        if (list[i].repo == "heading") {
            ...
            newH3.innerHTML = list[i].engName;
            document.getElementbyId('checkboxes').appendChild(
                newH3);
        } else {
            var newDiv = document.createElement('div');
            newDiv.innerHTML = '<img src="images/' + list[i].pakName +
                '.png" />"' + '<input class="' + list[i].repo +
                '" type="checkbox" id="' + list[i].pakName + '">' +
                list[i].engName + '</input>' + '<p>' + list[i].description +
                '</p><br />';
            document.getElementbyId('checkboxes').appendChild(
                newDiv);
        } //endif
